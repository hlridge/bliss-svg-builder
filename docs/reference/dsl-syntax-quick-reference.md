...
A space (`TSP` or `QSP`) separates words and is not a shape a character can be composed from: written in a `;`-slot it is dropped with a `MISPLACED_SPACE_PART` warning (any coordinate or option on it drops too) and the rest of the character renders. For a positionable blank inside a character, use `ZSA` instead.
...
- A **character** option on a code that expands to a whole word, or a **part** option (`[opts]>`) on one, is dropped with a `MISPLACED_CHARACTER_OPTION` / `MISPLACED_PART_OPTION` warning; the word still renders. A **word** option (`[opts]|`) on a code that expands to multiple words is dropped with `MISPLACED_GROUP_OPTION`, and all the words render. A bracket **without `>` in a `;`-part slot** (`B291;[color=red]B81`) is a misplaced character option: it is dropped with `MISPLACED_CHARACTER_OPTION` and the part renders bare (a part takes `[opts]>`).
